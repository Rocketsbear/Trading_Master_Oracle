...
> **💡 最低配置**：即使不配任何Key，系统也能运行 — 技术分析使用公开的Binance数据，不需要Key。LLM Key 只在"深度分析"和"AI Chat"功能中使用。
...
| 18 | **多空比逆向指标(V4)** | ±8 | Binance | 散户多空比极端失衡（>65%）时逆向操作 |
...
### Q: 数据从哪里来？
- **价格/K线**：Binance Futures 公开API（免费，无需Key）
- **多空比/OI/FR**：Binance + OKX + Bybit + Hyperliquid（免费）
